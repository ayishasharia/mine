...
node * find(int n)       //function to find vertex nide of a vertex
{ 
node *t=g;                      
while(t!=0 && t->data!=n)
t=t->next;
...
void addedge(int u,int v)     //function to add n edge
{
node *v1,*v2,*t;
v1=find(u);
v2=find(v);
...
void printgraph()          //funtion to print a graph
{
node *t1,*t2,*t3;
printf("\nVertex\tEdge List\n");
t1=g;
...
void topsortin(int n)      //inserting nodes in topological order list at begning
{
...
//emptyq - to test queue is not empty or not
int emptyq()
{
if(front==rear)
return 1;
...
//main fucntion to demonstrate graph searchs and topological sort
